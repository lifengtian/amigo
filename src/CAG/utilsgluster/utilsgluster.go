//utilsgluster is a whole exome pipeline run on a gluster cluster
//where database and tools are shared
//intermediate files are copied to gluster as checkpoint
//
package utilsgluster
...
func (cmd Cmd) Do(dryrun bool, c chan int) {
	CurrentCommand = time.Now().String() + " " + cmd.Command
	if dryrun {
...
func (w Workflow) Init(jsonFileName string, sampleName string, dryrun bool) {
	...
	w["LOCALTMPDIR"] = w["LOCALTMPDIR"] + "/" + sampleName
	w["Fastq"] = w["TMPDIR"] 
	w["sampleName"] = sampleName
	w["JAVAGATK"] = w["JAVABIN"] + " -Xmx6g -Djava.io.tmpdir=" + w["LOCALTMPDIR"] + " -jar " + w["GATKJAR"] + " -et NO_ET -K " + w["GATKKEY"]
	// Prepare interval, only make sense for exome
	w["UGinterval"] = " -L " + w["Exons"] + "  -L " + w["TargetBed"] + " --interval_padding 50 "

	//TODO`
	w.GetToolsDBs(dryrun)
	prepDirs := "mkdir -p " +  w["Fastq"] + " ; mkdir -p " + w["LOCALTMPDIR"]
...
func (work Workflow) GetToolsDBs(dryrun bool) {
	prepDirs := "mkdir -p " + work["PIPELINE"]
	RunIt(dryrun, []Cmd{{prepDirs,""}})

	if !IsFile (  work["PIPELINE"] + "/.dbs.done" ) {
		//tools := work["S3cmd"] + " -c " + work["S3cfg.pipeline"] + " get s3://cagpipelines/" + work["Tools"] + " " + work["PIPELINE"] + "/"
		tools := "cp /gluster/cagpipelines/" + work["Tools"] + " " + work["PIPELINE"] + "/"
		...
		dbs := "cp /gluster/cagpipelines/" + work["DBs"] + " " + work["PIPELINE"] + "/"
		RunIt(dryrun, []Cmd{{dbs, ""}})

		prepDBs := " cd " + work["PIPELINE"] + ";" + " tar jxvf " + work["DBs"] 
		RunIt(dryrun, []Cmd{{prepDBs, ""}})

		check := " touch " + work["fastq"] + "/.dbs.done"
		RunIt(dryrun, []Cmd{{check,""}})

	} else {
		fmt.Fprintln(os.Stderr, "Skip GetToolsDBs")
	}
...
//GetFastq: fetch fastqs from local drive or s3
func (work Workflow) GetFastq(dryrun bool) {
	...
	RunIt(dryrun, cmds)
		...
		fmt.Fprintln(os.Stderr,"FASTQs exist ", r1, r2)
	}
}
...
func (work Workflow) DoFastqc(dryrun bool) {
	...
	if ! IsFile(work["Fastq"] + "/.fastqc1.done") && ! IsFile(work["Fastq"] + "/.fastqc2.done") {
	...
	check :=   " touch " + work["Fastq"] + "/.fastqc1.done" + " ; touch " + work["Fastq"] + "/.fastqc2.done"
	cmds =[]Cmd{{check,""}  }
	RunIt(dryrun,cmds)
	} else {
		fmt.Fprintln(os.Stderr,"Skip Fastqc")
	}
}
...
func (work Workflow) DoBWAAlnPE(dryrun bool) {
	...
	if ! IsFile( work["Fastq"] + "/.bwaaln1.done" ) && ! IsFile( work["Fastq"] + "/.bwaaln2.done" ) {
	bwa1 := work["BWA"] + " aln  -t 4  " + work["Reference"] + " " + r1 + " > " + s1 
	bwa2 := work["BWA"] + " aln  -t 4  " + work["Reference"] + " " + r2 + " > " + s2

	cmds := []Cmd{{bwa1, ""}, {bwa2, ""} }
	RunIt(dryrun, cmds)

	check := " touch " + work["Fastq"] + "/.bwaaln1.done" + " ; " + " touch " + work["Fastq"] + "/.bwaaln2.done"
	RunIt(dryrun, []Cmd{{check,""}})
	} else {
		fmt.Fprintln(os.Stderr, "Skip Aln")
	}
}
...
func (work Workflow) DoBWASampe(dryrun bool) {
	...
	if ! IsFile( work["Fastq"] + "/.sampe.done" ) {
	sampe := work["BWA"] + ` sampe -r "@RG\tID:` + sampleName + `\tSM:` + sampleName + `" ` + work["Reference"] + " " + s1 + " " + s2 + " " + r1 + " " + r2 +
		" | " +
		work["SAMTOOLS"] + " view -h -S  - " +
		" | " +
		work["JAVABIN"] + " -Xmx4g  -jar " + work["PICARD"] + "/SortSam.jar I=/dev/stdin O=/dev/stdout SO=coordinate VALIDATION_STRINGENCY=SILENT QUIET=true COMPRESSION_LEVEL=0 TMP_DIR=" + work["LOCALTMPDIR"] + 
		" | " +
		...
		" CREATE_INDEX=true VALIDATION_STRINGENCY=SILENT QUIET=true TMP_DIR=" + work["LOCALTMPDIR"]

	cmds := []Cmd{{sampe, ""}}
	RunIt(dryrun, cmds)

	check :=  " touch " + work["Fastq"] + "/.sampe.done"
	RunIt(dryrun, []Cmd{{check,""}})

	} else {
		fmt.Fprintln(os.Stderr, "Skip Sampe")
	}
...
func (work Workflow) DoDedup(dryrun bool) {
	sampleName := work["sampleName"]
	if ! IsFile ( work["Fastq"] + "/.dedup.done" ) {
	dedup := work["JAVABIN"] + " -Xmx4g  -jar " + work["PICARD"] + "/MarkDuplicates.jar " + "I=" + work["TMPDIR"] + "/SID" + sampleName + ".sorted.bam " + "O=" + work["TMPDIR"] + "/SID" + sampleName + ".dedup.bam " + " M=" + work["TMPDIR"] + "/SID" + sampleName + ".dedup.metric " + " VALIDATION_STRINGENCY=SILENT CREATE_INDEX=true REMOVE_DUPLICATES=false TMP_DIR=" + work["LOCALTMPDIR"] 
	cmds := []Cmd{{dedup, ""}}
	RunIt(dryrun, cmds) 

	check :=  " touch " + work["Fastq"] + "/.dedup.done"
	RunIt(dryrun, []Cmd{{check,""}})

	} else {
		fmt.Fprintln(os.Stderr, "Skip Dedup")
	}

	flagstat := work["SAMTOOLS"] + " flagstat " + work["TMPDIR"] + "/SID" + sampleName + ".dedup.bam  > " + work["TMPDIR"] + "/SID" + sampleName + ".dedup.bam.flagstat" 

	hsmetrics := work["JAVABIN"] + " -Xmx4g  -jar " + work["PICARD"] + "/CalculateHsMetrics.jar " + "I=" + work["TMPDIR"] + "/SID" + sampleName + ".dedup.bam " + "O=" + work["TMPDIR"] + "/SID" + sampleName + ".dedup.bam.target_coverage " + " VALIDATION_STRINGENCY=SILENT " + " BI=" + work["TargetPicard"] + " TI=" + work["TargetPicard"] + " TMP_DIR=" + work["LOCALTMPDIR"]

	cmds2 := []Cmd{{flagstat, ""}, {hsmetrics, ""}}
	RunIt(dryrun, cmds2)
}

func (work Workflow) DoRealigner(dryrun bool) {
	sampleName := work["sampleName"]
	if ! IsFile( work["Fastq"] + "/.realigner.target.done" ) {
	...
	check := " touch " + work["Fastq"] + "/.realigner.target.done"
	RunIt(dryrun, []Cmd{{check,""}})

	} else {
		fmt.Fprintln(os.Stderr, "Skip RealignerTargetCreator")
	}
	if ! IsFile ( work["Fastq"] + "/.indelrealigner.done" ) {
	...
	cmds2 := []Cmd{{realigner2, ""}}
	RunIt(dryrun, cmds2)

	check2 := " touch " + work["Fastq"] + "/.indelrealigner.done"
	RunIt(dryrun, []Cmd{{check2,""}})

	} else {
		fmt.Fprintln( os.Stderr, "Skip IndelRealigner")
	}
}

func (work Workflow) DoBQSR(dryrun bool) {
	sampleName := work["sampleName"]
	if ! IsFile ( work["Fastq"] + "/.baserecalibrator.done" ) {
	bqsr1 := work["JAVAGATK"] +
		" -R " + work["Reference"] +
	...
	check := " touch " + work["Fastq"] + "/.baserecalibrator.done"
	RunIt(dryrun, []Cmd{{check,""}})
	
	} else {
		fmt.Fprintln( os.Stderr, "Skip BaseRecablibrator")
	}

	if ! IsFile ( work["Fastq"] + "/.printreads.done"  ) {
	bqsr2 := work["JAVAGATK"] +
		" -R " + work["Reference"] +
		" -I " + work["TMPDIR"] + "/SID" + sampleName + ".dedup.indelrealigner.bam " +
	...
	check2 :=  " touch " + work["Fastq"] + "/.printreads.done"
	RunIt(dryrun, []Cmd{{check2,""}})
	
	} else {
		fmt.Fprintln( os.Stderr, "Skip PrintReads")
	}
}

func (work Workflow) DoReduceBAM(dryrun bool) {
	sampleName := work["sampleName"]
	if ! IsFile ( work["Fastq"] + "/.reducereads.done"  ) {
	...
	check := " touch " + work["Fastq"] + "/.reducereads.done"
	RunIt(dryrun, []Cmd{{check,""}})
	} else {
		fmt.Fprintln(os.Stderr, "Skip ReduceReads")
	}
}
...
func (work Workflow) CopyBack(dryrun bool) {
	//copy back
	excludes := "--exclude dedup.bam --exclude indelrealigner.bam --exclude dedup.bai --exclude indelrealigner.bai --exclude sorted.bai --exclude sorted.bam --exclude .sai "
	//--exclude .vcf --exclude .fq.gz "
...
func (work Workflow ) States () string {
